...
mvn test -P Regression - to kick off regression. We have Regression profile in pom.xml file.
 Profiles let's to have multiple build configurations.
 Also, regression profile runs RegressionRunner.java class.
 RegressionRunner class has list of features that were selected for regression.
...
//    </profiles>
    ...
    mvn clean verify -P - delete target folder, run cucumber test,
    generate cucumber HTML report or whatever was specified in Regression profile.
    */
...
//    </profiles>
 ...
 <threadCountMethods>2</threadCountMethods> - means 2 webdrivers will be created, 2 browsers will open,
  2 tests will be running at the same time.
    private static ThreadLocal<WebDriver> driverPool = new ThreadLocal<>();
    ThreadLocal class allows to create copy of webdriver object per thread at run time.
    If wouldn't use ThreadLocal, webdrivers would override each other.
...
#########################################################################
    ...
    BDD emerged from and extends TDD. Instead of writing unit tests from specification why
    not make the specification a test itself. The main idea is that business analysts,
    project managers, users or anyone without technical, but with sufficient business,
    knowledge can define tests.
    ...
    We store test scenarios in feature files. Also, we follow Page Object Model design pattern.
    ...
    It helps to organize code, specifically webelements and separate test from page logic.
     ...
     Besides pages package, we have package with step definitions, where we store code
     implementation for cucumber test steps. Then we have utilities package,
     where we store reusable methods and Driver class. For configuration, we use properties file.
