...
What is application server? - program that runs web application and makes it available
for user via browser. For example Tomcat, JBoss, Jetty, etc.
...
You cannot use same commands as me in Powershell or CMD, so just open GIT BASH (WINDOWS ONLY)
and we will be on the same track.
...
For Mac: run pwd to get path of current location, or just drag and drop project folder into
terminal to get path to it. put cd before to navigate there.
...
If CucumberRunner didn't fail, there is nothing to execute for FailedRunner. But,
the problem is that they share reports. Thus, cucumber.json file will be overridden
by FailedRunner and it will be empty.
